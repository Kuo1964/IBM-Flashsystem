...
def process_result(indent):
    i = " " * indent
    return f"""{i}if res.get("error"):
{i}    if u in global_visited: global_visited.remove(u)
{i}    print(f"  ❌ 抓取失敗: {{u}} ({{res['error']}})")
{i}else:
{i}    chunks = res.get("chunks", [])
{i}    links = res.get("links", [])
{i}    filtered = []
{i}    for c in chunks:
{i}        text = c.get("text", "")
{i}        h = hashlib.md5(text.encode()).hexdigest()
{i}        if h not in global_hashes:
{i}            global_hashes.add(h)
{i}            filtered.append(c)
{i}    if filtered:
{i}        add_chunks_to_db(filtered)
{i}        total_new_chunks += len(filtered)
{i}        source_name = filtered[0]["metadata"]["source"]
{i}        manifest[source_name] = {{"type": "web", "url": u, "chunks": len(filtered)}}
{i}    print(f"  ├─ [已解析] {{u}} (過濾後寫入 {{len(filtered)}}/{{len(chunks)}} Chunks) (發現 {{len(links)}} 新連結)")
{i}    for l in links:
{i}        if l not in global_visited and l not in url_queue:
{i}            url_queue.append(l)
{i}pages_processed += 1
{i}with open(visited_file, "w") as f: json.dump(list(global_visited), f)
{i}with open(hashes_file, "w") as f: json.dump(list(global_hashes), f)
{i}with open(queue_file, "w") as f: json.dump(url_queue, f)
{i}with open(manifest_path, "w", encoding="utf-8") as f: json.dump(manifest, f, indent=4)"""

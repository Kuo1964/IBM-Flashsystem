    ...
    while queue and len(visited_urls) < max_pages:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                html_content = ""
                # 首先嘗試 HTTP GET
                response = httpx.get(clean_target_url, headers=headers, timeout=30.0, follow_redirects=True)
...
def download_pdf_safely(pdf_url: str):
    ...
    try:
        ...
        response = httpx.get(pdf_url, headers={"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)"}, timeout=60.0, follow_redirects=True)

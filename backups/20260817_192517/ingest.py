...
def _url_worker_standalone(url: str, max_depth: int, max_pages: int, global_visited_list: list, global_hashes_list: list) -> Dict[str, Any]:
    """頂層獨立子進程網頁爬蟲工作者：隔離 Playwright Node.js IPC 管道，支援 ProcessPoolExecutor"""
    try:
...
def run_ingestion(mode: str = "all", force: bool = False, max_depth: int = 50, max_pages: int = 5000):
    """
    執行全自動增量掃描與資料更新 (具備多進程 Spawn 隔離與模式分流)
    :param mode: 'all' (全部), 'pdf-only' (僅處理 PDF), 'url-only' (僅處理網頁)
    :param force: 是否強制重新處理未變更的檔案/網址
    """
    print("=" * 60)
    ...
    # 2. 處理網頁 URLs (多進程 Spawn 代理池並行掃描)
    if mode in ["all", "url-only"] and config.RAW_URLS_FILE.exists():
        with open(config.RAW_URLS_FILE, "r", encoding="utf-8") as f:
            urls = [line.strip() for line in f if line.strip() and not line.startswith("#")]
        
        print(f"\n🌐 掃描到 {len(urls)} 個技術文檔網址，啟動 4 個獨立 Spawn 代理並行抓取...")
        ...
        try:
            with ProcessPoolExecutor(max_workers=4, mp_context=spawn_ctx) as executor:
                futures = {}
                for url in urls:
                    url_key = f"url_{url}"
                    url_hash = calculate_file_hash_text(url)

                    if not force and manifest.get(url_key, {}).get("hash") == url_hash:
                        print(f"  [跳過] 網址未變更: {url}")
                        continue

                    print(f"  🌐 [指派任務] 分發給獨立子進程: {url}")
                    future = executor.submit(_url_worker_standalone, url, max_depth, max_pages, list(global_urls), list(global_hashes))
                    futures[future] = (url_key, url, url_hash)

                for future in as_completed(futures):
                    url_key, url, url_hash = futures[future]
                    try:
                        res = future.result()
                        if res.get("status") == "success":
                            ...
                            print(f"  [警告] 網址處理失敗 ({url}): {res.get('error')}")
                    except Exception as e:
                        print(f"  [異常] 代理執行異常 ({url}): {e}")
        finally:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="IBM FlashSystem 知識庫增量更新管線")
    ...
    run_mode = "pdf-only" if args.pdf_only else ("url-only" if args.url_only else "all")
    run_ingestion(mode=run_mode, force=args.force)
